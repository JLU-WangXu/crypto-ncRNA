...
def apply_rna_secondary_structure(codon_sequence):
    ...
    # 验证索引长度
    if len(indices_order) != len(base_sequence):
        raise ValueError(f"生成的索引序列长度 ({len(indices_order)}) 与输入序列长度 ({len(base_sequence)}) 不匹配")
    ...
    if len(new_sequence) % 3 != 0:
        raise ValueError(f"重排序后的序列长度 ({len(new_sequence)}) 不是3的倍数")
